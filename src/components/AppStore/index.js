...
class AppStore extends Component {
  ...
  getFiltering = () => {
    ...
    const filteredApps = appsList.filter(
      eachDetails => eachDetails.category === activeId,
    )
    return filteredApps
  }
  ...
  render() {
    const {activeId} = this.state
    const filteredApps = this.getFiltering()
      ...
      <div className="background">
        ...
        <div className="apps">
          ...
          <ul className="display1">
            {tabsList.map(eachTable => (
              <TableItem
                tableItem={eachTable}
                key={eachTable.tabId}
                onChange={this.clickTabItem}
              />
            ))}
          </ul>
        </div>
